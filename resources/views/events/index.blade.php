<x-app-layout>
    <x-slot name="header">
        <span class="flex space-x-3 justify-between">
            ...
            @can('if_admin')
                <Link modal href="{{ route('events.create') }}" class="border rounded px-4 py-1">
                New Event
                </Link>
            @endcan
        </span>
    ...
    <div class="flex flex-col space-y-5">
        ...
        @elsecan('if_visitor')
            @php
                $events = \App\Models\Event::where('target', \App\Models\Event::TARGET_VISITOR)
                    ->where('visible_by_target', true)
                    ->get();
            @endphp
        @elsecan('if_company')
            @php
                $events = \App\Models\Event::leftJoin('event_participant', 'events.id', '=', 'event_participant.event_id')
                    ->where('event_participant.user_id', null)
        ...
        @foreach ($events as $event)
            <div class="flex flex-col space-y-4 pb-5 shadow-md bg-white rounded-md">
                ...
                <main class="flex flex-col space-y-4">
                    ...
                    @if ($formsGroups->count() > 0)
                        <section class="flex flex-col space-y-5">
                            <h5 class="font-medium text-md px-5 flex space-x-2"><svg xmlns="http://www.w3.org/2000/svg"
                                ...
                                @can('if_admin')
                                    <span>Forms groups</span>
                                @elsecan('if_company')
                                    <span>Subscribe to one of the following forms groups to participate in this
                                        event.</span>
                                @elsecan('if_visitor')
                                    <span>Fill one of the following froms groups to participate in this
                                        event.</span>
                                @endcan
                            </h5>
                            <div class="flex flex-col">
                                @foreach ($formsGroups as $group)
                                    <div class="px-5 py-2 flex justify-between items-center">
                                        <span
                                            class="px-4 py-1 bg-gradient-to-r from-sky-200/60">{{ $group->name }}</span>
                                        @canany(['if_company', 'if_visitor'])
                                            <div class="flex space-x-4">
                                                <Link href="{{ route('events.create') }}" method="GET"
                                                ...
                                                @can('if_company')
                                                    <Link
                                                        href="{{ route('subscribe', ['event' => $event, 'group' => $group]) }}"
                                                        method="POST"
                                                        class="text-white text-sm px-2 py-1 bg-blue-500 rounded flex space-x-2 items-center">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                        viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                                        class="w-5 h-5">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    @else
                        @can('if_admin')
                            <x-hint class="px-5" text="Select some forms groups bellow." />
                        @endcan
                    @endif
                </main>
                @can('if_admin')
